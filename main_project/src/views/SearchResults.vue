    ...
    <div  style="border-bottom: 1px solid grey; height: 100px; margin-top: 20px; margin-left: 5px; margin-right: 25px; padding-left: 10px; text-align: left; font-size: 30px">
        ...
        <!-- <div class="searchbar1">
          <v-autocomplete
            ...
            :menu-props='{ nudgeTop: 5,maxHeight: 150}'

            @update:search="updateSearch"
            @click="searchClass"
            @mousedown="searchClass"
            @keydown="searchClass"
            attach
            return-object
      ...
      <v-container>
        <v-row no-gutters>
          <v-col>
            ...
            <p class="text-left" align-self="center">Course: {{review.course_name}}</p>
            <p class="text-left" align-self="center">Professor: {{review.professor_name}}</p>
          </v-col>

          <v-col align-self="center">
            <p class="text-center text-subtitle-1 font-weight-regular">Course Rating</p>
            <v-rating
              class="pa-2 ma-2"
...
export default{
    ...
    methods: {
          ...
          try {
              ...
              if (!this.query){
                if(localStorage.getItem('schoolID')){
                  temp = `http://18.191.156.68:8000/api/reviews/?is_active=1&school=` + localStorage.getItem('schoolID');
                } else {
                  temp = 'http://18.191.156.68:8000/api/reviews/?is_active=1';
                }
              } else {
                var departments = ""
                var courses = ""
                ...
                console.log(departments + " | " + courses + " | " + professors);
                if(localStorage.getItem('schoolID')){
                  temp = `http://18.191.156.68:8000/api/reviews/?is_active=1&school=` + localStorage.getItem('schoolID') + '&courses=' + courses + '&professors=' + professors + '&departments=' + departments;
                } else {
                  temp = 'http://18.191.156.68:8000/api/reviews/?is_active=1&courses=' + courses + '&professors=' + professors + '&departments=' + departments;
                }
              ...
              this.reviews.forEach(review => {
                ...
                this.searchscoreDiff += review["difficulty"];
                this.searchscoreWork += review["workload"];
              ...
              if(this.reviews.length > 0) {
                ...
                this.searchscoreDiff = Math.trunc(this.searchscoreDiff * 10/this.reviews.length) / 10;
                this.searchscoreWork = Math.trunc(this.searchscoreWork * 10/this.reviews.length) / 10;
              }

              

              this.yData =[this.ratingOne, this.ratingTwo, this.ratingThree,this.ratingFour,this.ratingFive];
              // this.numRows = Math.floor(this.reviews.length / 5);
              // this.numColRem = this.reviews.length % 5;
              // console.log(this.numRows);
              // console.log(this.numColRem);
              // if(this.search){
              //   this.searchClass();
              // } else {
              //   this.searchresult = this.reviews;
              // }
      ...
      created() {
        ...
        this.query = JSON.parse(localStorage.getItem('searchQuery'));
