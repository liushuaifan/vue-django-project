  ...
  <v-card>
    <v-menu location="end">
      <template v-slot:activator="{ props }">
        <v-btn class="float-right ma-5" color="rgb(70,175,152)" v-bind="props" icon="mdi-dots-vertical"></v-btn>
      </template>
    ...
    <v-container>
      <v-row no-gutters>
        <v-col align-self="center">
          <p class="text-left text-h5 text-wrap ml-7 font-weight-medium text-medium-emphasis">Dining Hall: <a href="javascript:void(null);" class="text-decoration-underline" @click="searchThing(review.dining_hall_name, review.dining_hall, 0)">{{ review.dining_hall_name }}</a></p>
        </v-col>
    ...
    <v-container>
      <v-row no-gutters>
        ...
        <v-col v-if="review.taste" align-self="center">
          <p class="text-center text-subtitle-1 font-weight-bold">Taste</p>
          <v-rating
            class="pa-2 ma-2 text-body-2"
        ...
        <v-col v-if="review.cleanliness" align-self="center">
          <p class="text-center text-subtitle-1 font-weight-bold">Cleanliness</p>
          <v-rating
            class="pa-2 ma-2 text-body-2"
            v-model="review.cleanliness"
            empty-icon="mdi-circle-outline"
            full-icon="mdi-circle"
            color="rgb(70,175,152)"
            size="60"
      ...
      <br/>
      <v-row no-gutters>
        <v-col v-if="review.variety" align-self="center">
          <p class="text-center text-subtitle-1 font-weight-bold">Variety of Options</p>
          <v-rating
            class="pa-2 ma-2 text-body-2"
            v-model="review.variety"
            empty-icon="mdi-circle-outline"
            full-icon="mdi-circle"
            color="rgb(70,175,152)"
            size="60"
        ...
        <v-col v-if="review.staff_friendliness" align-self="center">
          <p class="text-center text-subtitle-1 font-weight-bold">Staff Friendliness</p>
          <v-rating
            class="pa-2 ma-2 text-body-2"
            v-model="review.staff_friendliness"
            empty-icon="mdi-circle-outline"
            full-icon="mdi-circle"
            color="rgb(70,175,152)"
            size="60"
  ...
  export default{
      ...
      data(){
         ...
         return {
          ...
          showModifiers: false,
          waitConversion: {
            '0': '0-5 minutes', '1': '5-10 minutes',
            '2': '10-15 minutes', '3': '15+ minutes'
          },
      ...
      methods: {
        async getData() {
            try {
                ...
                this.showModifiers = (this.review.poster_name == this.curUsername) && this.curActive;

                if (!this.review.is_active && localStorage.getItem('isAdmin') === 'false'){
                  this.$router.replace('/DiningVisitorPage');
                }
                
                const res2 = await axios.get(`http://18.191.156.68:8000/api/dining/comments/?review=` + this.$route.params.reviewId );
                this.comments = res2.data;

                try {
                  await axios.post('http://18.191.156.68:8000/api/dining/reviews/' + this.$route.params.reviewId + '/vote/', {
        ...
        async addComment(){
          if (this.curVerified){
            try {
                ...
                const response = await axios.post('http://18.191.156.68:8000/api/dining/comments/', {
                    poster: this.curUser,
        ...
        async clearvote() {
          await axios.delete(`http://18.191.156.68:8000/api/dining/reviews/` + this.$route.params.reviewId + '/vote/', {
        ...
        async reportUser(){
          ...
          if (this.reportReason.length != 0) {
            try {
              await axios.post('http://18.191.156.68:8000/api/dining/reviewreports/', {
        ...
        editReview(){
          this.$router.push('/EditDiningReview/' + this.$route.params.reviewId);
        },
        async deleteReview(){
          await axios.delete(`http://18.191.156.68:8000/api/dining/reviews/` + this.$route.params.reviewId + '/');
